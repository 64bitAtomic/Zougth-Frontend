...
import { getAllSeriesAPI } from "../../api/series.api";
...
export default function SeriesList() {
  ...
  const fetchSeries = async () => {
    ...
    try {
      ...
      const res = await getAllSeriesAPI(params);
  ...
  useEffect(() => {
    const timeout = setTimeout(() => { setPage(1); fetchSeries(); }, 400);
    return () => clearTimeout(timeout);
  }, [search]);

  useEffect(() => { setPage(1); fetchSeries(); }, [watchStatus, sortBy, order]);
  ...
  return (
    <div className="min-h-screen bg-gray-950 px-4 py-10">
      <SEO
  title="Series"
  description="Track your web series collection."
/>
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            >
              ...
              <option value="seriesName">Sort: Name</option>
        ...
        {!loading && series.length === 0 && (
          <div className="text-center py-20">
            ...
            <Link to="/series/add" className="text-violet-400 text-sm mt-2 inline-block hover:text-violet-300">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
            {series.map((s) => (
              ...
              >
                ...
                <div className="p-3 space-y-2">
                  <h3 className="text-white text-sm font-medium leading-tight line-clamp-2">{s.seriesName}</h3>
                  <div className="flex items-center justify-between">
                    {s.releaseYear && <span className="text-gray-500 text-xs">{s.releaseYear}</span>}
                    <span className={`text-xs px-2 py-0.5 rounded-full border ${STATUS_COLORS[s.watchStatus]}`}>
                      {s.watchStatus}
                    </span>
                  </div>
                  {s.totalEpisodes && <p className="text-gray-600 text-xs">{s.totalEpisodes} eps</p>}
                </div>
              </div>
            ))}
          </div>
